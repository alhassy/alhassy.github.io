...
(loop for total = (length blog/tags)
      ...
      do
      (let ((inhibit-message t)) (shell-command (concat "cd ~/blog; git add " (blog/make-tags-page :tag tag))))
...
(cl-defun blog/publish-current-article ()
  "Place HTML files in the right place, update archives, index, rss, tags; git push!"
  (interactive)
