...
| =blog/publish-current-article/multiple= | Full publish cycle (export → posts.json → index → git push) |
...
=blog/publish-current-article=, =blog/create-posts-json-file=, and =blog/new-post=.
...
=my/blog-mode=).  It dispatches on =#+article_style:= and handles everything —
...
(speed key =P= in =my/blog-mode=) is the fast path.
