...
Is this disastrous situation true?
+ I have everything in a single directory??
+ Why not have a ~src~ directory consisting of Org source text,
  a ~resources~ directory consisting of CSS and JS and image & pdf & video & gif &
  fontsassests,
  then an output ~public~ directory that copies over statics and produces HTML?  

-------------------------------------------------------------------------------- 

<<outline-container-orgdb4bfb9>>
...
| =blog--publish-multiple-articles= | Orchestrates per-heading exports for a container |
...
** blog--compute-posts, blog--refresh-posts                           :details:

=posts.json= was originally introduced as a caching layer but the caching was
never implemented — it was written then immediately read back, never consumed
outside Emacs, and committed to git on every publish. We axe it and compute the
post list directly from the org files (17 files — trivially fast).
