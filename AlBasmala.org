...
(cl-defun blog/publish-current-article ()
  ...
  ;; (shell-command (format "cd ~/blog; git add %s %s" (buffer-file-name) article))
  (message "Placed article html into the published blog directory")
  ;; (update-index-and-things)
  ;; (message "Updated index.html, tags, archive, and rss to now include this new article")
  ))

(cl-defun update-index-and-things ()
  "Invoke this when happy with an article (preview) and you're about to git commit."
  (interactive)
  (org-static-blog-assemble-index)
...
and provides a Disqus comments sections. For details, consult the `org-static-blog-post-postamble' function."
      (pcase o-label
        ("header" (concat
                   ...
                   (or (ignore-errors (format "<br><center><h1>%s</h1></center>" (org-static-blog-get-title (buffer-file-name)))) "")))
...
:END:
#+begin_src emacs-lisp
(defun my/blog/style-setup (_backend)
  "Insert blog header (fancy title), tags, blog image (before “* Abstract”), and footer (links to tags)."
    ...
    ;; For now, I must explicitly write #+begin_abstract...#+end_abstract, due to how I've defined
...
:PROPERTIES:
:CUSTOM_ID: Inserting-doc-org-link-blog-seamlessly-via-the-export-process
:END:

#+begin_src emacs-lisp
(defun my/blog/style-setup/disable ()
  (interactive)
  (remove-hook 'org-export-before-processing-hook #'my/blog/style-setup))

(defun my/blog/style-setup/enable ()
  (interactive)
  (add-hook 'org-export-before-processing-hook  #'my/blog/style-setup))

;; In AlBasmala.el, I'll do the following as a default.
(my/blog/style-setup/enable)
#+end_src

** Finally, preview with every save
:PROPERTIES:
:CUSTOM_ID: Finally-preview-with-every-save
:END:
#+begin_src emacs-lisp
  ...
  (let ((org-header (concat "#+EXPORT_FILE_NAME: " pub-filename
        ...
        (abstracts-of-posts (--map
                             (concat
                              ...
                              ;; ⟨3⟩ Preview (and disable how abstracts look)
                              "\n#+HTML: <style> div.abstract {background-color: pink} <style>\n"
