...
*** Vendoring redirects so CI can resolve them                       :details:
...
to point at the in-repo copy:

- *Local path* (=~/.emacs.d/init.org=, =../foo.org=, …) — =url-copy-file='d
  into =resources/redirects/<slug>.org=, which is git-tracked.
- *HTTP(S) URL* (=https://raw.githubusercontent.com/…/README.org=, …) —
  fetched into the same directory.

After vendoring, the property itself is rewritten to =resources/redirects/<slug>.org=.
CI then sees an in-tree path and the existing =#+include:= machinery just
works, without special-casing.

=blog--vendor-redirects= runs on =C-x C-s= in multiple-style buffers (see
=my/blogging-mode-map=): preview is when redirect targets change, and when
the author can resolve "that URL is not reachable" errors on the spot
rather than discover them in CI.

The staleness check still applies unchanged — =blog--subtree-stale-p=
compares =:MODIFIED:= against the vendored copy's mtime, which updates
every time =blog--vendor-redirects= re-copies the upstream.
...
- =rss.xml= — every post, newest first.  For the reader who wants
  everything you write.
- =<tag>-rss.xml= — per-tag feed (e.g. =emacs-rss.xml=, =arabic-rss.xml=).
...
compare the feed's current GUIDs against that seen-set:

- /New GUID/ → reader announces it as a new post.
- /Already-seen GUID/ → reader ignores the item, /even if its title,
  description, or pubDate changed/.

We use the article URL (=alhassy.com/<slug>=) as the GUID.  This has a
direct consequence: *editing an already-published article does not
re-notify subscribers*.  Fix a typo in =arabic.org=, rewrite a paragraph,
add a whole new section — CI regenerates =rss.xml= on the next push, but
the item's GUID is unchanged, so readers silently keep the updated
content in place without waking anyone up.  That's the RSS consensus, by
the way: if every spelling fix re-notified thousands of readers, feeds
would be unusable.

If you want subscribers to hear about an edit, you have two honest options:

1. *Write a short "Updated: X" post.*  A standalone article, or a new
   heading in a container, whose =#+description= says what changed and
   whose body links back to the revised original.  New slug, new URL,
   new GUID — every reader announces it.  This is what most blogs do.

2. *Break the slug on purpose.*  Rename the file or set =:SLUG:= to
   something like =arabic-v2=.  The old URL now 404s (or redirects),
   the new one is a fresh GUID, and the feed re-announces.  Noisy, but
   appropriate if the revision is a true do-over.

We don't currently expose a =:RSS_REVISION:= knob that would rebuild the
GUID as =alhassy.com/<slug>#revN= — if the "quiet by default" behaviour
ever becomes painful, that's a two-line addition to =blog--make-one-rss-feed=.
...
The one apparent exception is container-style files: =blog--subtree-stale-p=
compares a subtree's =:MODIFIED:= against =<slug>.html= on disk, and
short-circuits re-export when the HTML is newer.  But on a fresh CI
checkout there is no HTML on disk, so =(file-exists-p html-file)= is
always =nil= and the short-circuit never fires.  The staleness check is
interactive-preview machinery; CI re-exports every subtree regardless.
