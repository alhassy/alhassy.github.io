...
7. TODO: Consider just making AlBasmala live in ~/blog/posts, I don't think
   there's any real reason for breaking consistency.
   
   - Need this conditional since AlBasmala lives in ~/blog whereas usually
     articles live in ~/blog/posts.
...
- *=:SLUG:= property* → pins the URL slug explicitly, bypassing the auto-derived
  ...
  the heading title changes, or when the auto-slug is awkward.  Uniqueness is your
  responsibility; no dedup suffix is appended.
...
*** Slug derivation and the JSON cache
...
| =blog--publish-single-subtree= | Exports one heading → =~/blog/<slug>.html= + =<slug>.org.html=; stamps =:MODIFIED:= |
| =blog--subtree-stale-p= | Predicate: returns non-nil when a subtree's HTML is out of date |
| =blog--publish-multiple-articles= | Orchestrates per-heading exports; skips up-to-date subtrees |
| =blog-publish-multiple= | Full publish cycle (export → posts.json → index → git push) |
...
| =blog--compute-posts-and-pages= | Scans posts/*.org; returns =(posts . pages)= separating =:SITE_NAV: t= subtrees |
...
scanning =~/blog/posts/*.org= directly.  Its sole heavyweight consumer is
...
=my/blogging-mode=).  It dispatches on =#+article_style:= and handles everything —
HTML export, colourised source, targeted tag page rebuild, index, git commit,
and push.  For iterating on one heading inside a container, =blog-publish-current-subtree=
...
(blog--refresh-posts)
#+end_src
* Orphan HTML Validation
:PROPERTIES:
:CREATED:  [2026-05-04 Mon 13:05]
:END:

Second, once you have your publishing list configured, it can be
difficult to keep your /org and /html directories in sync. Org Mode has
a built-in solution for this: =ox-publish= uses a timestamp cache
controlled by =org-publish-use-timestamps-flag=. When enabled (it's on
by default), the =org-publish-needed-p= function checks each file's
modification time against the last export and skips anything that hasn't
changed. This can speed up the build process, but I've often had trouble
with it and opted to simply delete the HTML contents and start fresh.

Also, it has no way of cleaning up orphaned files. Let's say you delete
or rename an =.org= file, the corresponding =.html= file will sit in the
build directory, basically outdated and no longer being used but still
there.

I've solved that problem here by wiping/deleting the html directory
contents on each build via the build.el script. Though, this can
increase your build time if you have a large amount of .org files.
That's why I'd recommend keeping separate notebooks organized by topic.
