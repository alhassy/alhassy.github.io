...
**** Should this be in the ci or locally during preview phase? 
:PROPERTIES:
:CREATED:  [2026-05-05 Tue 15:27]
:END:
...
| =blog--validate-no-orphan-html= | Warns/errors when =X.html= or =X.org.html= has no matching slug or =posts/X.org= |
| =blog-publish-multiple= | Full publish cycle: validates slugs, exports each subtree, regenerates index → git push |
...
| =blog--compute-posts-and-pages= | Scans ~/blog/*.org; returns =(posts . pages)= separating =:SITE_NAV: t= subtrees |
...
=blog-publish-current-article=, =blog--compute-posts=, and =blog-new-post=.
It reads =#+article_style:= via the same regex approach used throughout =blog--info=,
so the two styles are always in sync.
...
When =blog-publish-multiple= processes this heading it synthesises the usual
...
Previously every invocation of =blog-publish-multiple= unconditionally
re-exported every heading — correct, but increasingly slow for large containers.
...
Both branches push the slug into =results= so the calling =blog-publish-multiple=
still refreshes the index, RSS, and tag pages for /all/ articles — just without
touching HTML that is already current.
...
scanning =~/blog/*.org= directly.  Its sole heavyweight consumer is
=blog-make-tags-page=, which iterates it to build every article card on the
...
Publishing is a single command: =M-x blog-publish-current-article= (or =C-c C-p= in
=my/blogging-mode=).  It exports the article HTML + colourised source, commits the
=.org= source and the generated artefacts, and pushes to =master=.  That is all it
does — index.html, tag pages, and RSS are deliberately left to CI (see
§[[Deployment via CI]]).  No point regenerating them on your laptop when CI will
rebuild everything from scratch moments after the push anyway.

For iterating on one heading inside a container, =blog-publish-current-subtree=
(speed key =P= in =my/blogging-mode=) is the fast path.

For *standalone* articles the flow is straightforward (one file → one HTML file).

For *multiple-style container* files, =blog-publish-current-article= delegates to
=blog--publish-multiple-articles=, which iterates over every non-=:noexport:= top-level
heading and calls =blog--publish-single-subtree= for each.

=blog--publish-single-subtree= is the core of the container export pipeline.  See
§[[The Two Article Styles: standalone vs multiple]] for a full explanation of the
synthetic temp-file trick it uses to route every sub-article through the unchanged
=blog--style-setup= pipeline.  The short version: we build a temp =.org= file with the
right =#+keyword:= lines, paste the promoted subtree body into it, and run
=org-html-export-to-html= exactly as for a standalone post.
...
point doing that work locally."
  ...
  (-let [base (f-base (buffer-file-name))]
    ...
    (when (equal base "AlBasmala") (org-babel-tangle))
...
Local =C-c C-p= only exports the article HTML(s) and pushes the source.  Index and
tag pages are not regenerated locally — there is no point, since CI will rebuild
everything from scratch moments after the push.

*One-time GitHub setup:* in the repo Settings → Pages, set the publishing source
branch to =gh-pages=, directory =/= (root).

** Asset sync                                                        :details:

=blog--sync-assets= copies =resources/= into =public/= so the deployed
subtree is self-contained.  Since all articles now live at =~/blog/= alongside
=resources/=, image references in exported HTML are simply =resources/foo.png= —
the same path that resolves from the deployed root.
