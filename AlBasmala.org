...
because it works without the author having to remember to stamp anything.
...
- *No =:MODIFIED:= present* → guid is just the URL → edits are silent.
  Typos, whitespace fixes, paragraph rewrites all land on the deployed
  HTML without waking anyone up.
- */Bumping/ =:MODIFIED:=* (to any later ISO date, e.g. =2026-05-06=) →
...
(defun blog--refresh-posts ()
  ...
  (let ((result (blog--compute-posts-and-pages)))
    ...
    (setq blog-tags
          (sort (seq-uniq (-flatten (seq-map (lambda (it) (s-split " " (map-elt it "tags") t))
                                             blog-posts)))
                #'string<))
...
"<link href=\"resources/org-modern-export.css\" rel=\"stylesheet\" type=\"text/css\" />" (ref:modernCSS)
...
+ [[(modernCSS)][org-modern-export.css]] badge:||success|org-modern-export.css|css3 ::

  Recreate the [[https://github.com/minad/org-modern][org-modern]] look --- dotted bullets, TODO pills, tag
  chips --- on the exported HTML, see §[[Org-Modern-Export]].
...
#+begin_details "Full, tangled, value of blog-page-header"
...
#+begin_src css -r -n :tangle resources/blog-banner.css :noeval -n
...
** Pink Tables :details:
#+begin_src css :tangle resources/blog-banner.css :noeval -n
table {
     background: pink;
     border-radius: 10px;
     /* width:90% */
...
#+begin_src css -n  :tangle resources/floating-toc.css :noeval
...
* Ξ: Org-Modern-style Export Styling
  :PROPERTIES:
  :CUSTOM_ID: Org-Modern-Export
  :END:

The [[https://github.com/minad/org-modern][org-modern]] Emacs package gives buffers a gorgeous look --- dotted
bullets on headings, TODO/DONE pills, tag chips, pretty checkboxes. It
achieves this entirely via display-time text properties, which is
demonstrative of Emacs' strength but means /none of it survives HTML
export/. Since we want the /web/ to look the same as the /buffer/, we
recreate the effect in pure CSS, targeting the classes Org's HTML
exporter already emits.

By way of example:
+ Each =* Heading= becomes =<div class="outline-1"><h2>…</h2></div>=,
  so we prepend a bullet on =.outline-N h(N+1)::before=, cycling
  through =◉ ○ ◆ ◇ ★ ☆= by depth.
+ =TODO= and =DONE= keywords come out as =<span class="todo TODO">= and
  =<span class="done DONE">=; we style them as coloured pills.
+ Tag groups come out as =<span class="tag"><span class="foo">foo</span>…</span>=;
  each chip gets a rounded grey background.
+ Org renders ASCII checkboxes as =<code>[X]</code>=, and the
  surrounding =<li>= carries =class="on"=, =class="off"=, or
  =class="trans"=; we swap in Unicode ballots via =::before=.

#+begin_details "CSS for org-modern-style export"
#+begin_src css -n :tangle resources/org-modern-export.css :noeval
/* org-modern-style export — recreate the buffer look on exported HTML. */

/* Dotted bullets on headings, cycling by depth.  Scoped to .outline-N
   so the top-banner <h1 class="post-title"> is not affected. */
.outline-1 > h2::before,
.outline-text-1 h3::before { content: "◉ "; color: #c678dd; }
.outline-2 > h3::before { content: "○ "; color: #56b6c2; }
.outline-3 > h4::before { content: "◆ "; color: #98c379; }
.outline-4 > h5::before { content: "◇ "; color: #e5c07b; }
.outline-5 > h6::before,
.outline-6 > h6::before { content: "★ "; color: #e06c75; }
...
.todo { background: #e06c75; color: #fff; }
.done { background: #98c379; color: #fff; }

/* Per-keyword overrides, in case finer grain is wanted. */
.WAITING { background: #e5c07b; color: #1a1a1a; }
.CANCELLED { background: #5c6370; color: #fff; }
...
/* Checkboxes: swap ASCII for Unicode ballots.
   Org emits <li class="on"><code>[X]</code>…</li> (similarly off, trans). */
li.on > code:first-child,
li.off > code:first-child,
li.trans > code:first-child {
  display: none;
}
li.on::marker  { content: "☑ "; color: #98c379; }
li.off::marker { content: "☐ "; color: #777; }
li.trans::marker { content: "⊟ "; color: #e5c07b; }

/* Timestamps: monospace, muted. */
...
#+begin_src css :tangle resources/blog-banner.css :noeval
