...
Publishing is a single command: =M-x blog/publish-current-article=.  It dispatches on
=#+article_style:= and then handles everything — HTML export, colourised source,
tag pages, index, =posts.json=, git commit, and push.
