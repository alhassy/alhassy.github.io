...
posts — all dated entries (including site_nav ones), sorted newest-first.
      ...
      (when (s-ends-with? ".org" file)
        (let ((infos (if (equal "multiple" (blog--article-style file))
          ...
          (dolist (info infos)
            ...
            (when (map-elt info "date")     (push info posts)))))))
...
"
      (pcase o-label
        ("header" (concat
                   ...
                   (thread-last (@title (blog--info (buffer-file-name)))
            ...
            (if redirect
                (with-current-buffer tmp-buf
                  ...
                  (insert (format "#+include: \"%s\"\n"
                                  (expand-file-name redirect (file-name-directory container-file))))
                  (save-buffer))
              (save-excursion
                (goto-char heading-point)
                (org-copy-subtree))
